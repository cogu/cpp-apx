...
namespace apx_test
{
   ...
   TEST_F(FileCacheTest, NodeWithoutPorts)
   {
      ...
      std::array<std::uint8_t, NODE_VERSION_HEADER_SIZE + node_name_size + vm::UINT8_SIZE + SHA256_HASH_SIZE + vm::UINT8_SIZE*2> expected = {
   ...
   TEST_F(FileCacheTest, NodeWithUint8ProvidePort)
   {
      ...
      constexpr std::uint8_t expected_number_of_require_ports = 0u;
      constexpr std::size_t node_name_size = 10u; //Includes null-terminator char
      constexpr std::uint8_t provide_port_data_size = 20u;
      std::array<std::uint8_t, NODE_VERSION_HEADER_SIZE + node_name_size + vm::UINT8_SIZE + SHA256_HASH_SIZE + vm::UINT8_SIZE * 2 + provide_port_data_size> expected = {
         'A', 'P', 'X', NODE_HEADER_MAJOR_VERSION, NODE_HEADER_MINOR_VERSION, 'V','M', vm::MAJOR_VERSION, vm::MINOR_VERSION,
         ...
         expected_number_of_provide_ports, expected_number_of_require_ports,'P', 0x01, 0x03,
         'U', 'I', 'n', 't', '8', 'P', 'o', 'r', 't', '\0',
         'C', '(', '0', ',', '3', ')', '\0'
      };
